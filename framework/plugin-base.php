<?php
namespace moritzjacobs\PluginBase;
...
class Main {
	 ...
	 * @return void
	 */
	protected function add_tools_page($title=false, $callback=false) {
		if (!$title) {
			$title = get_called_class() . " " . __("Tools");
		}
		
		if(!$callback) {
			$callback = array($this, "load_tools_page");
		}
	...
	private function admin_js($files) {
		...
		if ($this->is_plugin_screen()) {
			foreach($files as $file) {
				$css_slug = Utils::sluggify(str_replace(".css", "", basename($file)));
				add_action( 'admin_enqueue_scripts', function() use ($file, $slug, $css_slug, $version){
					wp_enqueue_script($slug .'-admin-'.$css_slug, plugins_url($file, dirname(__FILE__)), array(), $version);
				});
			}
		}
	}
